...
@pytest.mark.parametrize(
    ...
    [
        (settings.test_user_email, settings.test_user_password, HTTPStatus.CONFLICT),
...
def test_unauthorized_permissions(test_client: Client, session: Session, faker: Faker):
    """
    ...
    assert response.status_code == HTTPStatus.UNAUTHORIZED
    response = test_client.put(f"/v1/users/{user_id}/roles/008ba9ea-4613-4466-a666-bc4a6617397f/")
    assert response.status_code == HTTPStatus.UNAUTHORIZED
    response = test_client.delete(f"/v1/users/{user_id}/roles/008ba9ea-4613-4466-a666-bc4a6617397f/")
    assert response.status_code == HTTPStatus.UNAUTHORIZED


def test_for_forbidden_permissions(test_client: Client, session: Session, faker: Faker):
    """
    GIVEN Admin is not logged in
...
def test_put_user_roles(test_client: Client, session: Session, faker: Faker) -> None:
    """
    GIVEN admin user is logged in
    WHEN DELETE request is sent
    THEN HTTP code 200 is returned
...
def test_delete_user_roles(test_client: Client, session: Session, faker: Faker) -> None:
    """
    GIVEN admin user is logged in
    WHEN DELETE request is sent
    THEN HTTP code 200 is returned
